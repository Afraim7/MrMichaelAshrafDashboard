        ...
        if (state is AdminExamPublished || state is AdminExamUpdatesSaved) {
          ...
          AppHelper.showSuccessBar(
            ...
            message: widget.existingExam != null
                ? AppStrings.success.examUpdated
        ...
        icon: AnimatedSwitcher(
          ...
          child: Icon(
            ...
            color: AppColors.skyBlue.withOpacity(
              widget.existingExam != null ? 1.0 : 0.3,
            ),
          ),
        ),
      ),
      ...
      IconButton(
        icon: Icon(
          ...
          color: AppColors.posterRed.withOpacity(
            (widget.existingExam != null && isEditing) ? 1.0 : 0.3,
          ),
        ),
                              ...
                              children: [
                                ...
                                if ((question['options'] as List).length > 2)
                                  IconButton(
                                    onPressed: () {
                                      setState(() {
                                        // If removing the correct answer option, clear the selection
                                        if (currentCorrectIndex == optIndex) {
                                          question['correctAnswerIndex'] = null;
                                        } else if (currentCorrectIndex !=
                                                null &&
                                            currentCorrectIndex > optIndex) {
                                          // Adjust index if removing an option before the selected one
